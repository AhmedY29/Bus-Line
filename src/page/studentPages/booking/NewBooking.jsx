      ...
      {currentStep === "search" && (
        <>
          <Card className="border-0">
            <CardHeader><CardTitle><Search className="w-6 h-6 mr-2" />Find Your Trip</CardTitle></CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                <Select value={searchFilters.destinationId} onValueChange={(value) => setSearchFilters(f => ({ ...f, destinationId: value }))}>
                  <SelectTrigger disabled={loading.options}><SelectValue placeholder="Destination" /></SelectTrigger>
                  <SelectContent>
                    {destinations.map((dest) => (
                      <SelectItem key={dest._id} value={dest._id}>{dest.title}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                ...
                <Select value={searchFilters.passengers.toString()} onValueChange={(value) => setSearchFilters(f => ({ ...f, passengers: parseInt(value) }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{[1, 2, 3, 4, 5].map((num) => <SelectItem key={num} value={num.toString()}>{num} Passenger{num > 1 ? "s" : ""}</SelectItem>)}</SelectContent>
                </Select>
                <Button onClick={handleSearch} className="w-full">Search Trips</Button>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-4">
            ...
            {getFilteredTrips().map((trip) => (
              <Card key={trip._id} className="border-0 hover:shadow-lg transition-shadow">
                <CardContent className="p-6">
                  <div className="flex flex-col lg:flex-row justify-between gap-6">
                    <div>
                      <div className="flex items-center gap-3 mb-3">
                        <Badge variant="outline">{trip.busNumber || "BUS-00X"}</Badge>
                        <div className="flex items-center gap-1">{renderStars(trip.driverId.rating)}<span>({trip.driverId.rating?.toFixed(1) || "New"})</span></div>
                      </div>
                      <h3 className="font-bold text-lg">{trip.neighborhood} → {trip.destinationId.title}</h3>
                      <p className="text-gray-600">Driver: {trip.driverId.name}</p>
                    </div>
                    <div className="text-right space-y-2">
                      <div>
                        <span className="text-2xl font-bold">{trip.tripPrice} SAR</span>
                        <p className="text-sm text-gray-600">per person</p>
                      </div>
                      <Button onClick={() => handleSelectTrip(trip)}><ArrowRight className="w-4 h-4 ml-2" />Book Now</Button>
                    </div>
                  </div>
                </CardContent>
              </Card>
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="border-0">
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                <Label className="text-lg font-semibold mb-2 block">Select Payment Method</Label>
              ...
              <div className="flex justify-between pt-6">
                <Button variant="outline" onClick={() => setCurrentStep("search")}>Back</Button>
