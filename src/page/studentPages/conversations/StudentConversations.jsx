import { MessageCircle, Search, Phone, Video, MoreVertical, Send, ArrowLeft } from "lucide-react";
  ...
  return (
    <div className="max-w-7xl mx-auto h-[calc(100vh-400px)]">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            ...
            <div className="flex items-center space-x-2">
              <Badge className="bg-blue-100 text-blue-700">
                {conversations.reduce((total, conv) => total + conv.unread, 0)} unread
              </Badge>
            </div>
          </div>
                    ...
                    >
                      <div className="relative">
                        <Avatar className="h-12 w-12">
                          <AvatarImage src="/placeholder.svg" alt={conversation.driverName} />
        ...
        {selectedConversation && (
          <div className={`lg:col-span-2 ${selectedConversation ? 'block' : 'hidden'} flex flex-col h-full`}>
            {/* Chat Header */}
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        <h2 className="text-lg font-semibold">{selectedConversation.driverName}</h2>
                        {selectedConversation.status === 'active' && (
                          <div className="w-2 h-2 bg-green-500 rounded-full" />
                        )}
                      </div>
                      <p className="text-sm text-gray-600">
                        {selectedConversation.tripRoute}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
            ...
            <Card className="bus-card border-0 rounded-t-none border-t-0">
              <CardContent className="p-4">
                ...
                <div className="flex items-center justify-between mt-3 text-xs text-gray-500">
                  <span>{selectedConversation.status === 'active' ? 'Driver is online' : 'Driver is offline'}</span>
